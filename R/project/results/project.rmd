...
```{r}
plot_pair = function (row)
    ggplot(wine_quality_data_no_outlier, 
           aes_string(toString(corr_weak[row,]$row), toString(corr_weak[row,]$column))) +
        geom_point()

myplots <- lapply(seq(1, 4), plot_pair)
...
```{r}
subset(corr_df, abs(corr_df['cor_max']) >= 0.3 & abs(corr_df['cor_max']) < 0.5)[, -which(names(corr_df) == "cor_max")]
```

### Strongly correlated variables

```{r}
subset(corr_df, abs(corr_df['cor_max']) >= 0.5)[, -which(names(corr_df) == "cor_max")]
```
# Multiple variables
