...
outliers <- function(df, label, powerstart=-10, powerend=10, tukey_quiet=TRUE) {
    
    x <- df[[label]]
    x_t <- transformTukey(x, start=powerstart, end=powerend, plotit=FALSE, quiet=tukey_quiet)
    rosner <- rosnerTest(x_t, k=10)
    outliers <- subset(rosner$all.stats$Obs.Num, rosner$all.stats$Outlier == TRUE)
    noutliers <- length(outliers)
    
    cat('Number of outliers for', label, ':', noutliers, '\n')
...
Some potential outliers were noted in the single variable analysis. Here outliers are identified by Rosner's test and removed before looking at correlations between two variables. Rosner's test assumes the data are from a normal distribution, so first a transformation is done using Tukey's Ladder of Powers to make the data more normal. For variables with outliers, box plots are created.


```{r}
outlierlist <- c()
for (i in colnames(wine_quality_data)){
   o <- outliers(wine_quality_data, i, -66, 1)
   outlierlist <- union(outlierlist, o)
}
```
```{r}
length(outlierlist)
