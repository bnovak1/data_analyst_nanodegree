...
r_df <- data.frame(
    ...
    r = (r)[ut],
    rho = (rho)[ut])
r_df['cor_max'] = sign(r_df['r'])*pmax(abs(r_df['r']), abs(r_df['rho']))
```
...
```{r}
subset(r_df, abs(r_df['cor_max']) >= 0.1 & abs(r_df['cor_max']) < 0.3)
```

### Moderately correlated variables

```{r}
subset(r_df, abs(r_df['cor_max']) >= 0.3 & abs(r_df['cor_max']) < 0.5)
```

### Strongly correlated variables

```{r}
subset(r_df, abs(r_df['cor_max']) >= 0.5)
```
# Multiple variables
